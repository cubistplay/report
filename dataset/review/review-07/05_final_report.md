...
feat(review-07): list promotion candidates는 Update DB의 active memory를 key별로 묶어 promotion threshold를 충족한 후보를 PromotionCandidate로 반환합니다. 최초 PR c74ffbe은 brainwash/memory/update_db.py에 61줄을 추가했고, 기존 전체 test 122건을 통과했습니다.
...
| 단계 | Commit | 내용 | 검증 |
| --- | --- | --- | --- |
| 최초 PR | c74ffbe | promotion candidate Value Object 및 조회 | 전체 122건 통과 |
| 리뷰 명세 | 6df5fcd | stable record order·normalized target test | 초기 구현에서 2 failures 확인 |
| 리뷰 반영 | 9234442 | 정렬 기반 grouping 및 target canonicalization | update DB 12건, 전체 124건 통과 |
